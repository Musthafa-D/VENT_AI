...
class Explanation:
    ...
    def visualize_node(self, 
        ...
        if heat_by_exp:
            if self.node_imp is not None:
                print(f"node importance not none: {self.node_imp}")
                print(f"len of node importance: {len(self.node_imp)}")
                node_c = []
                for i in subG.nodes:
                ...
                draw_args['node_color'] = node_c
                draw_args['cmap'] = plt.cm.coolwarm
                draw_args['vmin'] = min(node_c)
                draw_args['vmax'] = max(node_c)
            ...
            if self.edge_imp is not None:
                ...
                draw_args['edge_cmap'] = plt.cm.plasma
                draw_args['edge_vmin'] = min(edge_heat)
                draw_args['edge_vmax'] = max(edge_heat)
        ...
        nx.draw(subG, pos, ax = ax, **draw_args, 
                with_labels = show_node_labels, font_color='w', node_size = 500,
                arrows=True, arrowstyle='->')

        # Highlight the center node index:
        nx.draw(subG.subgraph(self.node_idx), pos, node_color = 'black', 
                node_size = 600, ax = ax)
        ...
        if 'node_color' in draw_args:
            sm = plt.cm.ScalarMappable(cmap=plt.cm.coolwarm, norm=plt.Normalize(vmin=min(draw_args['node_color']), vmax=max(draw_args['node_color'])))
            sm._A = []
            cbar = plt.colorbar(sm, ax=ax, label='Node Importance')
            cbar.set_ticks([])

        if 'edge_color' in draw_args:
            sm = plt.cm.ScalarMappable(cmap=plt.cm.plasma, norm=plt.Normalize(vmin=min(draw_args['edge_color']), vmax=max(draw_args['edge_color'])))
            sm._A = []
            cbar = plt.colorbar(sm, ax=ax, label='Edge Importance')
            cbar.set_ticks([])

        if show:
            plt.show()

    def visualize_graph(self, ax = None, show = False, agg_nodes = torch.mean, show_node_labels=False):
        '''
        ...
        if self.node_imp is not None:
            ...
            draw_args['cmap'] = plt.cm.plasma
            draw_args['vmin'] = min(node_imp_heat)
            draw_args['vmax'] = max(node_imp_heat)
        ...
        nx.draw(G, pos, ax = ax, **draw_args , with_labels = show_node_labels, font_color='w',
                arrows=True, arrowstyle='->')
    
        # Add color bar for nodes
        if 'node_color' in draw_args:
            sm = plt.cm.ScalarMappable(cmap=plt.cm.plasma, norm=plt.Normalize(vmin=min(draw_args['node_color']), vmax=max(draw_args['node_color'])))
            sm._A = []
            cbar = plt.colorbar(sm, ax=ax, label='Node Importance')
            cbar.set_ticks([])
    
        if 'edge_color' in draw_args:
            sm = plt.cm.ScalarMappable(cmap=plt.cm.coolwarm, norm=plt.Normalize(vmin=min(draw_args['edge_color']), vmax=max(draw_args['edge_color'])))
            sm._A = []
            cbar = plt.colorbar(sm, ax=ax, label='Edge Importance')
            cbar.set_ticks([])
