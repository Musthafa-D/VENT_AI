...
def plot_gnn_explainer(model, sample, node_idx, index, epoch, final_epoch):
    ...
    ax.set_title(f"{node_idx} Node Explanation of sample {index} of epoch {epoch}", fontsize=20, fontweight='semibold')
    figure_storage.store(
            fig, f"{node_idx} Node Explanation of sample {index} of epoch {epoch}")
    plt.show()
    
    if epoch == final_epoch and index == 0:
